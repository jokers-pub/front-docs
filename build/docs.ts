...
async function createPage(moudleName: string) {
    ...
    for (let file of docs) {
        ...
        await generatePage(examplesComponentRoot, pageRoot, fileName, result);
    }
}

async function generatePage(examplesComponentRoot: string, pageRoot: string, fileName: string, content: string) {
    let result: string[] = [];
    ...
    result.push("<script>");
    result.push(await prettierify(prettier, getScriptPart(examplesComponentRoot, pageRoot, fileName), "typescript"));
    result.push("</script>");
